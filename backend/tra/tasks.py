...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(crontab(hour=6, day_of_week=3), remove_old_feature_builds.s(), name='Delete older than last 3 FBs')
    sender.add_periodic_task(crontab(minute=17), 
...
def celery_pull_and_analyze_notanalyzed_testruns_by_regfilter(regression_filter_id, query_limit: int=None):
    pull_and_analyze_notanalyzed_testruns_by_regfilter(regression_filter_id=regression_filter_id, query_limit=query_limit)
