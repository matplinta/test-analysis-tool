...
class GetDataForFailChartBase(APIView):
    ...
    def init_analyzer(self, fail_message_dict, filters):
        ...
        if date_start and date_end:
            filters["end__gte"] = f'{date_start.strftime("%Y-%m-%d")} 00:00:00'
            filters["end__lt"] = f'{date_end.strftime("%Y-%m-%d")} 23:59:59'
...
class GetChartByExceptionType(GetDataForFailChartBase):
    ...
    def get(self, request):
        ...
        if self.is_dataframe_empty(analyzer):
...
class AllSubscribedTestSetFiltersBase(GetDataForFailChartBase):
    ...
    def prepare_filters_for_users_all_subs_regfilters(self):
        ...
        results = ["environment issue", "not analyzed", "passed"]
    ...
    def handle_creation_or_update_of_users_allsubs_filterset(self, filters):
        user_allsubs_filterset_name = f"AUTOGENERATED FROM SUBSCRIBED Test Sets"
...
class CreateUsersAllSubsFilterSet(AllSubscribedTestSetFiltersBase):

    @swagger_auto_schema(
        description="Create user's filterset matching all parameters from his/hers subscribed TestSetFilters",
        operation_description="Create user's filterset matching all parameters from his/hers subscribed TestSetFilters",
        responses={
    ...
    def _get_distinct_fail_message_type_groups_based_on_subs_tsfilters(self):
        subs_testsetfilters = tra_models.TestSetFilter.objects.filter(subscribers=self.request.user)
        used_fmtg = subs_testsetfilters.order_by("fail_message_type_groups").distinct("fail_message_type_groups").values_list("fail_message_type_groups", flat=True)
        used_fmtg = list(filter(lambda item: item is not None, used_fmtg))
        return ",".join([str(s) for s in used_fmtg])

    def _get_estimated_limit_based_on_subs_tsfilters(self):
        subs_testsetfilters = tra_models.TestSetFilter.objects.filter(subscribers=self.request.user)
        ti_len = len(tra_models.TestInstance.objects.filter(test_set__in=subs_testsetfilters))
        if ti_len == 0:
            return 100
        else:
            return ti_len * 14
    ...
    def get(self, request):
        filters = self.prepare_filters_for_users_all_subs_regfilters()
        filters["fail_message_type_groups"] = self._get_distinct_fail_message_type_groups_based_on_subs_tsfilters()
        # filters["limit"] = self._get_estimated_limit_based_on_subs_tsfilters()
        filterset = self.handle_creation_or_update_of_users_allsubs_filterset(filters)
        return Response(self._serialize_data_with_filters(filterset))
